...
const bootstrapAdminFromEnv = async () => {
  const username = String(
    process.env.BOOTSTRAP_ADMIN_USERNAME || DEFAULT_BOOTSTRAP_ADMIN_USERNAME
  ).trim();
...
const verifyAuthToken = (req, res, next) => {
  ...
  try {
    ...
    // Important: resolve current role from DB on each request.
    // This makes role changes effective immediately and prevents stale admin tokens.
    const user = db.central
    ...
    req.userRole = String(user.role || 'user');
    return context.runWithContext({ userId: user.id }, next);
  } catch (_error) {
...
app.post('/api/auth/start', async (req, res) => {
  try {
    ...
    if (!client) {
      ...
      const API_ID = String(process.env.API_ID || idRow?.value || '').trim();
      const API_HASH = String(process.env.API_HASH || hashRow?.value || '').trim();
